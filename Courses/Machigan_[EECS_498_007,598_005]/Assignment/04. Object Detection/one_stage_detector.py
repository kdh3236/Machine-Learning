...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for i in range(0, len(stem_channels)):
          if i == 0:
            conv = Conv2d(in_channels, stem_channels[0], 3, 1, 1, bias=True)
          else:
            conv = Conv2d(stem_channels[i-1], stem_channels[i], 3, 1, 1, bias=True)
          ...
          stem_box.append(Conv2d(stem_channels[i-1], stem_channels[i], 3, 1, 1))
        ...
        # Replace "pass" statement with your code
        self.pred_cls = Conv2d(stem_channels[-1], num_classes, 3, 1, 1)
        self.pred_box = Conv2d(stem_channels[-1], 4, 3, 1, 1)
        self.pred_ctr = Conv2d(stem_channels[-1], 1, 3, 1, 1)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for i in range(3, 6):
          cls = self.pred_cls(self.stem_cls(feats_per_fpn_level[f'p{i}']))
          B, C, H, W = cls.shape
          class_logits[f'p{i}'] = cls.permute(0, 2, 3, 1).reshape(B, -1 ,C)

          box = self.pred_box(self.stem_box(feats_per_fpn_level[f'p{i}']))
          B, C, H, W = box.shape
          boxreg_deltas[f'p{i}'] = box.permute(0, 2, 3, 1).reshape(B, -1 ,C)

          ctr = self.pred_ctr(self.stem_box(feats_per_fpn_level[f'p{i}']))
          B, C, H, W = ctr.shape
          centerness_logits[f'p{i}'] = ctr.permute(0, 2, 3, 1).reshape(B, -1 ,C)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_get_deltas_from_locations(
    ...
    for i in range(N):
      ...
      if torch.all(gt_boxes[i, :4] == -1):
        deltas[i, :] = torch.tensor([-1, -1, -1, -1], dtype=locations.dtype, device=locations.device)
      else:
        l = (xc - gt_boxes[i, 0]) / stride 
        t = (yc - gt_boxes[i, 1]) / stride
        r = (gt_boxes[i, 2] - xc) / stride
        b = (gt_boxes[i, 3] - yc) / stride
        deltas[i] = torch.tensor([l, t, r, b], dtype=locations.dtype, device=locations.device)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_apply_deltas_to_locations(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    l = deltas[:, 0].clamp_min(0)
    t = deltas[:, 1].clamp_min(0)
    r = deltas[:, 2].clamp_min(0)
    b = deltas[:, 3].clamp_min(0)
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    lr = torch.stack((l, r), dim = 1)
    tb = torch.stack((t, b), dim = 1)

    centerness = torch.sqrt((torch.min(lr, dim=1).values*torch.min(tb, dim=1).values) / torch.max(lr, dim=1).values*torch.max(tb, dim=1).values)
    mask = (deltas == -1).all(dim=1)
    centerness[mask] = -1
    """

    centerness = torch.zeros(deltas.shape[0], dtype=deltas.dtype, device=deltas.device)
    ...
    for i in range(deltas.shape[0]):
      if (torch.all(deltas[i, :] == -1)):
        ...
        centerness[i] = math.sqrt((min(l, r)*min(t, b)) / (max(l,r)*max(t,b)))      
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        # Replace "pass" statement with your code
        locations_per_fpn_level = get_fpn_location_coords(fpn_feats, self.backbone.fpn_strides, dtype=images.dtype, device=images.device)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        for i in range(B):
          ...
          # gt 하나에 대한 p3, p4, p5의 Dictionaty을 return
          matched_gt = fcos_match_locations_to_gt(
          ...
          # xc, yc를 이용하여 Deltas 계산
          matched_deltas['p3'] = fcos_get_deltas_from_locations(
            locations_per_fpn_level['p3'], matched_gt['p3'] ,self.backbone.fpn_strides['p3']
          )
          matched_deltas['p4'] = fcos_get_deltas_from_locations(
            locations_per_fpn_level['p4'], matched_gt['p4'] ,self.backbone.fpn_strides['p4']
          )
          matched_deltas['p5'] = fcos_get_deltas_from_locations(
            locations_per_fpn_level['p5'], matched_gt['p5'] ,self.backbone.fpn_strides['p5']
          )
        ...
        cls_ids = matched_gt_boxes[..., 4].long() # (B, N), Class label
        pos = (cls_ids != -1) # (B, N), Background가 아닌 부분

        # any() -> 적어도 하나 이상이 Background가 아닌 경우
        if pos.any():
            max_label = int(cls_ids[pos].max().item())
            if max_label >= C:# 0 ~ c-1의 범위의 Class label을 가져야 한다.
                # Backfround가 아닌 경우에만 c-1
                cls_ids = torch.where(pos, cls_ids - 1, cls_ids)
        # pos가 True -> cls_ids의 값, False라면 0
        # pos에 위치에 대응되는 값을 선택
        safe_ids = torch.where(pos, cls_ids, torch.zeros_like(cls_ids)) # (B, N)
        # one-hot vector 생성
        # clamp를 통해 0 ~ c-1로 값을 강제
        gt_classes = F.one_hot(
            safe_ids.clamp(min=0, max=C-1), num_classes=C
        ).to(pred_cls_logits.dtype)  # (B, N, C)
        gt_classes = gt_classes * pos.unsqueeze(-1).float()
        ...
        # (N, )
        matched_gt_centerness = fcos_make_centerness_targets(
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            level_pred_boxes[:, 0::2] = level_pred_boxes[:, 0::2].clamp(min=0, max=h-1)
            level_pred_boxes[:, 1::2] = level_pred_boxes[:, 1::2].clamp(min=0, max=w-1)
            ##################################################################
            #                          END OF YOUR CODE                      #
            ##################################################################
