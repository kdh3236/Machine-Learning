...
- $e_{t, i}$는 `Decoder`의 **$t$번째 `Hidden state`를 구할 때, `Encoder`의 $i$번째 `hidden state`가 얼마나 영향을 미치는 지를 나타낸다.**

- Decoder에서 각 Time step마다 전체 Hidden state를 보고, 현재 생성 / 번역해야하는 단어와 가장 관련있는 Hidden state를 찾는다.
...
> 먼저, `RNN+Attention`에서의 **`Decoder`의 `Previous Hidden state`를 `Query`가 대체하고, `Encoder`의 `Hidden state`를 $X$가 대체**한다.
