...
class RPNPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for i in range(0, len(stem_channels)):
          if i == 0:
            conv = nn.Conv2d(in_channels, stem_channels[0], 3, 1, 1, bias=True)
          else:
            conv = nn.Conv2d(stem_channels[i-1], stem_channels[i], 3, 1, 1, bias=True)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for i in range(3, 6):
          ...
          feat = self.stem_rpn(feats_per_fpn_level[level_name])

          B, C, H, W = feats_per_fpn_level[level_name].shape

          pred_obj = self.pred_obj(feat)
          pred_obj = pred_obj.permute(0, 2, 3, 1).reshape(B, -1)

          pred_box = self.pred_box(feat)
          pred_box = pred_box.permute(0, 2, 3, 1).reshape(B, -1, 4)

          object_logits[level_name] = pred_obj
...
def generate_fpn_anchors(
    ...
    for level_name, locations in locations_per_fpn_level.items():
        ...
        for aspect_ratio in aspect_ratios:
            ...
            ##################################################################
            # Replace "pass" statement with your code
            # Tensor로 만드는 것이 Data type의 일치, Device 종류 일치 측면에서 좋다.
            w = torch.tensor(math.sqrt(area / aspect_ratio), dtype=locations.dtype, device=locations.device)
            h = area / w
...
def iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    ...
    # Replace "pass" statement with your code   
    """
    완전히 벡터화 된 코드 (By Chat-GPT)
    # (M,)
    x1, y1, x2, y2 = boxes1.unbind(dim=1)
    area1 = (x2 - x1).clamp(min=0) * (y2 - y1).clamp(min=0)

    # (N,)
    X1, Y1, X2, Y2 = boxes2.unbind(dim=1)
    area2 = (X2 - X1).clamp(min=0) * (Y2 - Y1).clamp(min=0)

    # pairwise intersections -> (M, N)
    # None -> unsqueeze(1)과 비슷한 개념이라고 생
    inter_x1 = torch.maximum(x1[:, None], X1[None, :]) # (M,1) vs (1,N) → (M,N)
    ...
    eps = torch.finfo(boxes1.dtype).eps if boxes1.is_floating_point() else 1e-7
    return inter / (union + eps)
    """

    M = boxes1.shape[0]
    N = boxes2.shape[0]
    iou = torch.zeros((M, N), dtype=boxes1.dtype, device=boxes1.device)
    ...
    a_y1 = boxes1[:, 1]
    a_y2 = boxes1[:, 3]
    a_area = (a_x2-a_x1).clamp(min=0) * (a_y2-a_y1).clamp(min=0)

    for i in range(boxes2.shape[0]): # N
      gt_x1 = boxes2[i, 0]
      gt_x2 = boxes2[i, 2]
      gt_y1 = boxes2[i, 1]
      gt_y2 = boxes2[i, 3]
      ...
      # (M, )
      xx1 = torch.maximum(gt_x1, a_x1[:])
      xx2 = torch.minimum(gt_x2, a_x2[:])
      yy1 = torch.maximum(gt_y1, a_y1[:])
      yy2 = torch.minimum(gt_y2, a_y2[:])

      inter = (xx2 - xx1).clamp(min=0) * (yy2 - yy1).clamp(min=0) 

      iou[:, i] = inter / (a_area + gt_area - inter)      
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
    return iou
...
def rcnn_apply_deltas_to_anchors(
    ...
    output_boxes = None
    # Replace "pass" statement with your code
    bx = anchors[:, 0] + anchors[:, 2]*deltas[:, 0]
    by = anchors[:, 1] + anchors[:, 3]*deltas[:, 1]
    bw = anchors[:, 2] * torch.exp(deltas[:, 2])
    bh = anchors[:, 3] * torch.exp(deltas[:, 3])

    output_boxes = torch.stack((bx, by, bw, bh), dim=1)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
    return output_boxes
...
class RPN(nn.Module):
    ...
    def forward(
        ...
        locations = get_fpn_location_coords(
          shape_per_fpn_level, strides_per_fpn_level,device=device, dtype=dtype
        )

        anchors_per_fpn_level = generate_fpn_anchors(
          locations, strides_per_fpn_level, 
        ...
        for i in range(num_images):
          ...
          matched_gt_box = rcnn_match_anchors_to_gt(anchor_boxes, gt, self.anchor_iou_thresholds)
        ...
        if self.training:
            ...
            # Replace "pass" statement with your code
            # Foreground와 Background가 5:5가 되도록 분리
            fg_idx, bg_idx = sample_rpn_training(
                matched_gt_boxes,                      
                num_samples=self.batch_size_per_image * num_images,
                fg_fraction=0.5,
            )
            indices = torch.cat([fg_idx, bg_idx], dim=0)

            gt_deltas = rcnn_get_deltas_from_anchors(
              anchor_boxes[indices], matched_gt_boxes[indices]
            )
      
            # Matched_gt_boxes == -1이라면 Background
            # Foreground에 해당하는 것만 뽑아냄
            obj_targets = (matched_gt_boxes[indices, 4] >= 0).to(pred_obj_logits.dtype)
            loss_obj = F.binary_cross_entropy_with_logits(
                pred_obj_logits[indices], obj_targets, reduction="none"
            )

            loss_box = F.l1_loss(
                pred_boxreg_deltas[indices], 
                gt_deltas,               
                reduction="none"
            )
    ...
    def predict_proposals(
        ...
        for level_name in anchors_per_fpn_level.keys():
            ...
            for _batch_idx in range(level_obj_logits.shape[0]):
                ...
                scores = level_obj_logits[_batch_idx]
                deltas = level_boxreg_deltas[_batch_idx]
                ...
                # XYWH -> XYXY
                bx1 = (bx - 0.5 * pw).clamp(min=0, max=w - 1)
                by1 = (by - 0.5 * ph).clamp(min=0, max=h - 1)
                bx2 = (bx + 0.5 * pw).clamp(min=0, max=w - 1)
                by2 = (by + 0.5 * ph).clamp(min=0, max=h - 1)

                boxes = torch.stack((bx, by, bw, bh), dim=1)

                # Box 중 Score가 가장 큰 K개 선택
                k = min(scores.shape[0], self.pre_nms_topk)
                topk_scores, topk_idx = torch.topk(scores, k, dim=0)
                topk_boxes = boxes[topk_idx]

                # NMS를 통해 필요없는 Anchor Box 제거
                keep_topk_per_level = torchvision.ops.nms(topk_boxes, topk_scores, self.nms_thresh)
                # NMS 이후에 추가적으로 Box 개수 제한
                keep_topk_per_level = keep_topk_per_level[:self.post_nms_topk]
                
                level_proposals_per_image.append(topk_boxes[keep_topk_per_level])
                ##############################################################
                #                        END OF YOUR CODE                    #
                ##############################################################
...
class FasterRCNN(nn.Module):
    ...
    def __init__(
        ...
        for i in range(0, len(stem_channels)):
          if i == 0:
            conv = nn.Conv2d(self.backbone.out_channels, stem_channels[0], 3, 1, 1, bias=True)
          else:
            conv = nn.Conv2d(stem_channels[i-1], stem_channels[i], 3, 1, 1, bias=True)
        ...
        fc = nn.Linear(stem_channels[-1]*roi_size[0]*roi_size[1], num_classes+1, bias=True)
    ...
    def forward(
        ...
        for level_name in feats_per_fpn_level.keys():
            ...
            # Replace "pass" statement with your code
            # rot_aligned는 Box argument에 list[Tensor]를 입력 받음
            level_props = [b.to(level_feats.device).to(torch.float32) for b in level_props]

            roi_feats = torchvision.ops.roi_align(
              level_feats, level_props, output_size = self.roi_size, 
              spatial_scale = 1.0 / float(level_stride),  aligned = True
            )
            ##################################################################
            #                         END OF YOUR CODE                       #
            ##################################################################
        ...
        # cross_entropy()는 정수형 Index만 받음
        targets = (matched_gt_boxes[indices, 4] + 1).to(torch.int64)

        loss_cls = F.cross_entropy(
          pred_cls_logits[indices], targets, reduction="mean"
        )
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def inference(
        ...
        # Replace "pass" statement with your code
        pred_cls_scores = pred_cls_logits.softmax(dim=1)

        pred_scores, pred_classes = torch.max(pred_cls_scores, dim=1)


        # Only retain prediction that have a confidence score
        mask = (pred_scores > test_score_thresh) | (pred_classes == 0)
        pred_scores = pred_scores[mask]
        pred_classes = (pred_classes[mask] - 1).to(torch.int64)
