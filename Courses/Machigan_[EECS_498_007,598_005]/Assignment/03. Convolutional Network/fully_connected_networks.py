...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        ##################################################
        # Replace "pass" statement with your code
        # reshape 위해서 x의 차원을 미리 가져옴
        num_data, *input_shape = x.shape 
        ...
        dx = dout@w.T
        dx = dx.reshape(num_data, *input_shape)

        # b가 forward에서 broadcasting되었기 때문에
        # Upstream에 temp를 내적함
        # temp는 Local gradient = 1이므로, 전부 1인 원소를 
        # 가지게 하고 사이즈만 맞춰준다.
        temp = torch.ones(num_data, dtype=dout.dtype).unsqueeze(dim = 1).to(dout.device)
        db = dout.T@temp
        db = db.reshape(1, -1) 
        ##################################################
        #                END OF YOUR CODE                #
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ###################################################################
        # Replace "pass" statement with your code
        self.params['W1'] = torch.randn((input_dim, hidden_dim), dtype=torch.double, device='cuda:0') * weight_scale
        self.params['W2'] = torch.randn((hidden_dim, num_classes), dtype=torch.double, device='cuda:0') * weight_scale
        self.params['b1'] = torch.zeros(hidden_dim, dtype=torch.double, device='cuda:0')
        self.params['b2'] = torch.zeros(num_classes, dtype=torch.double, device='cuda:0')
        ###############################################################
        #                            END OF YOUR CODE                 #
        ###############################################################
    ...
    def loss(self, X, y=None):
        ...
        hidden = X_f@self.params['W1'] + self.params['b1'] # N * H
        relu = hidden.clamp(min=0) # N * H
        scores = relu@self.params['W2'] + self.params['b2'] # N * C
        ...
        ###################################################################
        # Replace "pass" statement with your code
        target_scores = softmax[torch.arange(X_f.shape[0]), y] # N

        # Loss: 정답 클래스의 Score의 Log를취했을 때의 평균
        ...
        grads['W2'] = relu.T@dsoftmax + 2 * self.reg * self.params['W2'] # H * C

        mask = (hidden > 0) # N * H
        drelu = dsoftmax@self.params['W2'].T # N * H    
        drelu = drelu * mask   

        grads['b1'] = drelu.sum(dim=0)

        grads['W1'] = X_f.T@drelu + 2 * self.reg * self.params['W1']    # D * H
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1, self.num_layers + 1):
          if i == 1:
            self.params[f'W{i}'] = weight_scale * torch.randn((input_dim, hidden_dims[0]), dtype=self.dtype, device=device)
            self.params[f'b{i}'] = torch.zeros(hidden_dims[0], dtype=self.dtype, device=device)
          elif i == self.num_layers:
            self.params[f'W{i}'] = weight_scale * torch.randn((hidden_dims[i-2], num_classes), dtype=self.dtype, device=device)
            self.params[f'b{i}'] = torch.zeros(num_classes, dtype=self.dtype, device=device)
          else:
            self.params[f'W{i}'] = weight_scale * torch.randn((hidden_dims[i-2], hidden_dims[i-1]), dtype=self.dtype, device=device)
            self.params[f'b{i}'] = torch.zeros(hidden_dims[i-1], dtype=self.dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        if self.use_dropout:
          for i in range(1, self.num_layers+1):
            if i == self.num_layers: # Last Layer
              scores = save[f'out{i-1}']@self.params[f'W{i}'] + self.params[f'b{i}'] # N * C
            else:
              if i == 1: # First Layer
                hidden = X@self.params['W1'] + self.params['b1'] # N * H1
                save[f'hidden{i}'] = hidden

                relu_mask = (hidden > 0)
                relu = relu_mask * hidden
                save[f'relu_mask{i}'] = relu_mask
                save[f'relu{i}'] = relu

                if 'seed' in self.dropout_param:
                  torch.manual_seed(self.dropout_param['seed'] + i)
              
                dropout_mask = (torch.rand(*relu.shape, device='cuda') < (1 - self.dropout_param['p'])).float()
                dropout = relu * dropout_mask / (1 - self.dropout_param['p'])
                save[f'dropout_mask{i}'] = dropout_mask
                
                save[f'out{i}'] = dropout
              else: # Intermediate Layer
                hidden = save[f'out{i-1}']@self.params[f'W{i}'] + self.params[f'b{i}']
                save[f'hidden{i}'] = hidden
                ...
                # i를 붙이는 이유는 Layer마다 다른 Seed를 갖도록 하여
                # Layer마다 다른 Mask를 갖도록 하기 위함이다.
                if 'seed' in self.dropout_param:
                  torch.manual_seed(self.dropout_param['seed'] + i)
        ...
        target_scores = softmax[torch.arange(X.shape[0]), y] # N
        ...
        if self.use_dropout:
          for i in range (self.num_layers, 0, -1):
            if i == 1:
              grads[f'b{i}'] = save_back[f'upst{i+1}'].sum(dim=0) # C
              grads[f'W{i}'] = X.T@save_back[f'upst{i+1}'] + self.reg * self.params[f'W{i}'] 
            else:
              if i == self.num_layers:
                grads[f'b{i}'] = dsoftmax.sum(dim=0) # C
                grads[f'W{i}'] = save[f'out{i-1}'].T@dsoftmax + self.reg * self.params[f'W{i}'] 

                ddropout = dsoftmax@self.params[f'W{i}'].T 
                ddropout = ddropout * save[f'dropout_mask{i-1}'] / (1 - self.dropout_param['p']) # N * H2

                drelu = ddropout * save[f'relu_mask{i-1}']

                save_back[f'upst{i}'] = drelu
              else:
                grads[f'b{i}'] = save_back[f'upst{i+1}'].sum(dim=0) # C
                grads[f'W{i}'] = save[f'out{i-1}'].T@save_back[f'upst{i+1}'] + self.reg * self.params[f'W{i}'] 

                ddropout = save_back[f'upst{i+1}']@self.params[f'W{i}'].T 
                ddropout = ddropout * save[f'dropout_mask{i-1}'] / (1 - self.dropout_param['p'])# N * H2
          ...
          for i in range (self.num_layers, 0, -1):
            if i == 1:
              ...
              if i == self.num_layers:
                ...
                drelu = dsoftmax@self.params[f'W{i}'].T 
                drelu = drelu * save[f'relu_mask{i-1}']
                ...
                drelu = save_back[f'upst{i+1}']@self.params[f'W{i}'].T 
                drelu = drelu * save[f'relu_mask{i-1}']
...
def rmsprop(w, dw, config=None):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    config['cache'] = config['cache'] * config['decay_rate'] + (1 - config['decay_rate']) * dw * dw
    next_w = w - dw * config['learning_rate'] / (config['cache'] + config['epsilon']).sqrt()
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def adam(w, dw, config=None):
    ...
    temp_m = config['m'] /(1 - config['beta1'] ** config['t'])
    temp_v = config['v'] /(1 - config['beta2'] ** config['t'])

    next_w = w - config['learning_rate'] * temp_m / (temp_v + config['epsilon']).sqrt() 
    #########################################################################
    #                              END OF YOUR CODE                         #
    #########################################################################
