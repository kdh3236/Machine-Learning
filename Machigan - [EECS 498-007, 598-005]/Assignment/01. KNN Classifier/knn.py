...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    # Broadcasting 이용
    x_train_m = x_train.view(num_train, -1)
    x_test_m = x_test.view(num_test, -1)

    for i in range(num_train):
      dists[i, :] = ((x_train_m[i] - x_test_m)**2).sum(dim = 1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_2 = x_train_m.pow(2).sum(dim = 1)
    x_test_2 = x_test_m.pow(2).sum(dim = 1)

    x_train_test = torch.matmul(x_train_m, x_test_m.T)

    dists = x_train_2[:, None] + x_test_2[None, :] - 2 * x_train_test
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    values, idx = torch.topk(dists, k, dim = 0, largest = False)

    label = torch.zeros_like(idx)

    for i in range(num_test):
      label[:, i] = y_train[idx[:, i]]
      # mode는 index까지 Tuple 형태로 반환
      y_pred[i] = torch.mode(label[:, i]).values
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        x_train_m = self.x_train.view(self.x_train.shape[0], -1)
        x_test_m = x_test.view(x_test.shape[0], -1)
...
def knn_cross_validate(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # chunk()는 tuple을 반환하기 떄문에, list로 변환까지 해야함
    ...
    for k in k_choices:
      ...
      for fold_idx in range(num_folds):
          ...
          x_train_cv = torch.cat([x_train_folds[i] for i in range(num_folds) if i != fold_idx])
          y_train_cv = torch.cat([y_train_folds[i] for i in range(num_folds) if i != fold_idx])
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k, accs in k_to_accuracies.items():
      t = torch.tensor(accs)
      mean_dict[k] = torch.mean(t)
    
    max = -1
    print(mean_dict)
  
    for k, m in mean_dict.items():
      if (m > max):
        max = m
        best_k = k
    ##########################################################################
