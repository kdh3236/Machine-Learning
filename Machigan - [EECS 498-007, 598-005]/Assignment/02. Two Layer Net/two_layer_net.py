...
def nn_forward_backward(
    ...
    total_scores = torch.sum(exp_scores, dim = 1) # N
    target_scores = exp_scores[torch.arange(X.shape[0]), y] # N

    total_softmax = -torch.log(target_scores / total_scores) # N
    softmax = exp_scores / total_scores.unsqueeze(1)

    # Loss 계산
    loss = torch.sum(total_softmax)
    loss /= X.shape[0]
    ...
    # W1
    grads["W1"] = torch.mm(X.T, dz) 
    grads["W1"] /= X.shape[0]
    grads["W1"] += 2 * reg * W1

    # b2
    grads["b2"] = softmax.sum(dim = 0) / X.shape[0]

    # b1
    grads["b1"] = dz.sum(dim = 0) / X.shape[0]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###################################!########################################
...
def nn_get_search_params():
    ...
    hidden_sizes = [256, 512, 1024, 4069]
      ...
      for hs in hidden_sizes:
        for rs in regularization_strengths:
          for lrd in learning_rate_decays:
            ...
            max_val_acc = max(stats["val_acc_history"])
