...
def svm_loss_vectorized(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    scores = torch.zeros(X.shape[0], W.shape[1], device='cuda:0')
    scores = torch.mm(X, W) # N * C

    # Trainind data index 용 (0 ~ N-1)
    idx = torch.arange(X.shape[0], device='cuda:0')

    # max_score: 각 Train data에서의 정답 Class score
    # unsqeeuze(1)은 Column 방향으로 Braodcasting할 때 필요
    max_score = scores[idx, y].unsqueeze(1) # (N, )

    one = torch.ones_like(scores, device='cuda:0')
    zero = torch.zeros_like(scores, device='cuda:0')

    # SVM Loss 계산
    svm_loss = scores - max_score + one # N * C
    # 정답 Class의 SVM_loss는 0으로 설정
    svm_loss[idx, y] = 0

    svm_loss = torch.maximum(svm_loss, zero)

    loss = torch.sum(svm_loss) / X.shape[0]
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    #############################################################################
    # Replace "pass" statement with your code
    # ( > 0) 방식은 Boolean type return, 숫자로 계산하려면 Type casting 필요
    mask = (svm_loss > 0).double() # N * C
    # X: N * D, W: D * C, Y: N

    mask_sum = mask.sum(dim = 1)
    
    # dW는 mask에서 true에 해당되는 만큼 X를 더해야 한다,
    # 하나의 Sample에서 정답 클래스에 해당되는 클래스는 
    # 그 Sample에서 잘못 예측한 만큼 빼야한다. 
    mask[idx, y] = -mask_sum[idx]
...
def sample_batch(
    ...
    #########################################################################
    # Replace "pass" statement with your code
    # randint의 size는 tuple 형태가 되어야 한다.
    indices = torch.randint(0, X.shape[0]-1, (batch_size, ))
    X_batch = X[indices, :]
    y_batch = y[indices]
    #########################################################################
    #                       END OF YOUR CODE                                #
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    # num_iter = 100 으로 테스트 해봤을 때, 
    # lr = 1e-2, reg = 1e-3 부근에서 최적의 정확도가 나오는 것을 확인
    learning_rates = [7e-3, 9e-3, 1e-2, 1.2e-2, 1.5e-2]
    regularization_strengths = [5e-4, 8e-4, 1e-3, 2e-3, 3e-3]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def softmax_loss_naive(
    ...
    for i in range(X.shape[0]):
      scores = torch.mv(W.T, X[i]) # C
      # Overflow 방지 정규화
      scores -= torch.max(scores)

      exp_scores = torch.exp(scores)
      total = torch.sum(exp_scores) 

      answer_score = torch.log(exp_scores[y[i]] / total)
      loss -= answer_score

      for j in range(W.shape[1]):
        softmax = torch.exp(scores[j]) / total
        
        if (j == y[i]):
          dW[:, j] += -X[i] + softmax * X[i]
        else:
          dW[:, j] += softmax * X[i]
