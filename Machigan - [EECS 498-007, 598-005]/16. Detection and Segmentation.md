...
`Box`를 **좌상단과 우하단 두 개**로 나타내고, `Ground Truth Box`에서 **좌상단 모서리에 특정 객체가 위치할 확률과 우하단 모서리에 특정 객체가 위치할 확률은 동일하다.**
...
좌상단을 표현하는 `Heatmap`과 우하단을 표현하는 `Heatmap`에서 **같은 객체를 가리키는 `좌상단 Pixel`과 `우하단 Pixel`을 연결**하기 위해, **쌍을 이루는 Pixel은 같은 `Embedding Vector`를 갖도록 학습한다.**
...
2. `Image`에서의 `Detection`은 보통 `High resolution image`에서 이루어지며, **`Convolution`을 `High resolution image`에서 수행하는 것은 계산 비용이 크다.**
...
- `Upsampling` 과정에서 사용되는 `Stride`는 보통 `Downsampling` 과정에서 사용된 `Stride`에 대응된다.
