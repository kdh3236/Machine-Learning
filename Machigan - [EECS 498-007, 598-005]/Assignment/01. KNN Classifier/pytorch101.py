...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x = torch.tensor([v for v in range(start, stop) if (v%10) == 0], dtype = torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    idx = [i for i in range(x.shape[0]-1, -1, -1)]
    y = x[idx, :]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    # temp를 구성해서 하는 방법
    """
    temp = torch.zeros(3, 2, 4)
    temp[0, 0, 0:4] = x[0:4]
    temp[0, 1, 0:4] = x[12:16]
    temp[1, 0, 0:4] = x[4:8]
    temp[1, 1, 0:4] = x[16:20]
    temp[2, 0, 0:4] = x[8:12]
    temp[2, 1, 0:4] = x[20:24]

    y = temp.view(3, 2*4)
    """

    temp1 = x.view(2, 3, 4)
    y = temp1.permute(1, 0, 2).contiguous().view(3, 2*4)
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    std = (((x - mean) ** 2).sum(dim = 0) / (x.shape[0] - 1)).sqrt()
